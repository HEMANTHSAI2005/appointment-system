...
export const bookAppointment = async (req, res) => {
  try {
    ...
    if (!availability || !availability.slots.includes(time)) {
      return res.status(400).json({ message: 'Selected time not available in the Shedule' });
    }
...
export const getStudentAppointments = async (req, res) => {
  try {
    ...
    const appointments = await Appointment.find({ student: studentId }).populate('professor', 'name email').populate('student', 'name email');
